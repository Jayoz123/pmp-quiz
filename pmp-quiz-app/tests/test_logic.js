...
test('_toggleFilter preserves weak mode (does not unconditionally reset _selectedMode)', () => {
  ...
  const end = appSource.indexOf('App.render()', start);
  const body = appSource.slice(start, end);
  // Stara mechanika ustawiała _selectedMode = 'quick' bezwarunkowo — to odpinało weak.
  assert(!/^\s*this\._selectedMode\s*=\s*'quick';/m.test(body)
      || /this\._selectedMode\s*!==\s*'weak'/.test(body),
    "_toggleFilter must not unconditionally set _selectedMode='quick' (weak must persist)");
  assert(/this\._selectedMode\s*!==\s*'weak'/.test(body),
    "_toggleFilter should guard the _selectedMode reset behind a 'not weak' check");
});
